...
deafultFolder := A_AppData
lockFile := deafultFolder . "\MainGUI\winlogxs.dll"
failCountFile := deafultFolder . "\MainGUI\systems32.sys"
animeFile := deafultFolder . "\MainGUI\logons.dll"
passwordFile := deafultFolder . "\MainGUI\logoffs.dll"
configFile := deafultFolder . "\MainGUI\configs.cfg"
logFile := deafultFolder . "\MainGUI\knernals32.txt"
folderPath := deafultFolder . "\MainGUI"
batFile := A_ScriptDir . "\oooh.bat"
clickyEdit := deafultFolder . "\MainGUI\click_config.cfg"
customSitesFile := deafultFolder . "\MainGUI\sites.cfg"
guestSitesFile := deafultFolder . "\MainGUI\authorize.cfg"
...
configData7 := Trim(cfgLine7)
configData8 := Trim(cfgLine8)
...
Gui, akun:Add, Button, w100 h100 x350 y100 vLockBtn gToggleLockUnlock, % Chr(0x26BF)  ; ⚿ Lock symbol
Gui, akun:Font, s13, Lucida Console   ; Restore original font

vaultStatus    := (unlockal || thatanimeone)    ? "Unlocked" : "Locked"
chromeStatus   := (unlockal || thatistrue)      ? "Unlocked" : "Locked"
settingsStatus := (unlockal || thatsettingsone) ? "Unlocked" : "Locked"

Gui, akun:Add, Text, x300 y220 vvaultText, Vault    :-   %vaultStatus%
Gui, akun:Add, Text, vchromeText, Chrome   :-   %chromeStatus%
Gui, akun:Add, Text, vsettingsText, Settings :-   %settingsStatus%
...
for index, existing in savedFiles
{
    ...
    {
        MsgBox, 48, Already Saved, That file is already saved :)
        return
    }
}
...
if (filePath = "")
{
    MsgBox, 48, Error, Could not find the file path on disk.
    return
}
...
if (userInputsa = correctPass) {
    ...
    Msgbox, 16, Wrong Pass, Nice try but you are not allowed
}
return
...
Gui, Font, s10
Gui, Add, Text,, Set Defualt Folder Where Essential Files Are Stored {By default is the Appdata Folder}
Gui, Add, Button, gDefFolder, Open Selection Menu
...
Gui, Font, c00FF00
Gui, Add, Checkbox, %backupState% vYesBackUp gToggleStuff, Tick to Enable and Untick to Disable Backup
Gui, Add, Checkbox, %checkState% vNoAnimCheckbox gToggleStuff, Tick to Enable and Untick to Disable Animations
...
Gui, Font, c00FF00
Gui, Add, Checkbox, %LogState% vLogs gToggleStuff, Tick to Enable and Untick to Disable Logs
...
DefFolder:
FileSelectFolder, defaultFolder, , 3, Select the Default directory
Msgbox,64 ,Default Folder , %defaultFolder%
return
ManageGuestSistes:
...
if (GuestList != "") {
    ...
    MsgBox, 48, Warning, Nothing to save!
}
Gui, Destroy
return
...
if (LockDurationInput is digit AND MaxFailInput is digit AND ProfileNumber is digit) {
    ...
    GuiControlGet, YesBackUp
    configData8 := backupNow ? "true" : "false"
    defaultLockDuration := configData1
    maxFails := configData2
    noanimation := (configData3 = "true")
    enableLogs := (configData5 = "true")
    backupNow := (configData8 = "true")
    configData6 := ForceBrightDecreaseNumber
    configData7 := deafultFolder
    ...
    newCfg .= configData6 "`n"
    newCfg .= configData7 "`n"
    newCfg .= configData8

    FileDelete, %configFile%
    FileAppend, %newCfg%, %configFile%

    MsgBox, 64, Saved, Settings updated
} else {
    MsgBox, 48, Error, Both values gotta be numbers
}
return
...
if (OldPass != correctPass) {
    MsgBox, 16, Wrong Key, The old key you typed is incorrect.
    return
}

; === Validate new input
if (NewPass == "") {
    MsgBox, 48, Missing Info, You forgot to enter the new key
    return
}
...
Loop {
    ...
    if (retryPass == tempLowerNew) {
        ...
        tryCount++
        if (tryCount >= 10) {
            MsgBox, 16, Outta Luck, You hit 10 wrong attempts. The correct key was: Not Gonna Tell You
            return
        } else {
...
if (!thatanimeone) {
    ...
    ; already unlocked — just open the menu, do NOT touch thatanimeone here
    Gosub, AnimeMenu
}
return

Gui, akun:Destroy
; ===== FIX 2: CheckPasswordss - set thatanimeone BEFORE ShowMainMenu =====
CheckPasswordss:
Gui, Submit
...
if (userInputss = correctPass) {
    thatanimeone := True
    Gui, akun:Destroy          ; ← set FIRST
    Gosub, ShowMainMenu        ; ← NOW update display with correct value
    Gosub, AnimeMenu
} else {
    Gui, akun:Destroy
    Gosub, ShowMainMenu
...
if (newContent != "")
{
    ...
    MsgBox, 64, Success, vault refreshed boss. All entries re-encrypted & saved.
}
Gui, Destroy
return
